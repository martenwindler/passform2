...
update msg model =
    case msg of
        ...
        SetCurrentAsDefault ->
            ...
            in
            ( { model | savedDefault = model.agents, logs = "Standardkonfiguration gespeichert." :: model.logs }
        ...
        StartPlanning _ ->
            case ( model.pathStart, model.pathGoal ) of
                ( Just start, Just goal ) ->
                    ...
                    in
                    ( { model | loading = True, logs = "Pfadplanung gestartet..." :: model.logs }
        ...
        PlanningResultRaw rawJson ->
            case Decode.decodeValue Decoders.pathDecoder rawJson of 
                Ok path -> ( { model | currentPath = Just path, loading = False, logs = "Pfad erfolgreich berechnet." :: model.logs }, Cmd.none )
                Err _ -> ( { model | loading = False, logs = "Fehler bei Pfadberechnung." :: model.logs }, Cmd.none )

        -- SYSTEM-LOGS & STATUS
        LogReceived text ->
            ( { model | logs = List.take 10 (text :: model.logs) }, Cmd.none )
